...
@auth
						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{ route('punbnewfinancing.index') }}">Search</a></li>
						...
						<form>
							<div class="row">
								<div class="col-md-6 col-sm-12">
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" value="{{ $financings->LADOFFICER }}" type="text"  disabled>
									</div>
								</div>
								<div class="col-md-6 col-sm-12">
						...
						<form>
							<div class="row">
								<div class="col-md-6 col-sm-12">
									...
									<label class="col-sm-12 col-md-12 col-form-label">Date Applied</label>
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ date('d-m-Y', strtotime($financings->DTAPPLIED)) }}" disabled>
									</div>
									
									
								</div>
								<div class="col-md-6 col-sm-12">
									<label class="col-sm-12 col-md-12 col-form-label">Application Reference No</label>
									<div class="col-sm-12 col-md-10">
											...
											<div class="pd-20">
												...
												<div class="clearfix mb-20">
													...
													<div class="pull-right">
														...
														<a href="{{route('punbnewfinancing.edit', [
															//$financings->ID, 
															$id, 
															'output' => 'print',
												...
												@if ($statuscase->count())
													<!-- disbaled -->
													<!-- <form action="{{ route('punbnewfinancing.update', $financings->ID )  }}" method="POST" name="updatehead">
														@csrf
														@method('PATCH')
														<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
														<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $financings->ID }}">
												...
												@else  
													@if( $financings->ICode == Auth::user()->company ||  Auth::user()->company == "PUNB" )
														@can('punb_financing-delete')
															<!-- active -->
															<form action="{{ route('punbnewfinancing.update', $financings->sis_id )  }}" method="POST" name="updatehead">
																@csrf
																@method('PATCH')
																<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $financings->ID }}">
	...
	@include('modal/punb/newfinancing/modalfinancing')
	@include('modal/modalupload')
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{ route('punbnewfinancing.edit', $id) }}",
				columns: [
		...
		$('body').on('click', '.ActiveItem', function () {
			...
			if (confirm("Are You sure want to unhide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('NFstatusActiveCase',$id) }}",
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/ViewCommentNF/'+comm_id, function (data) {
