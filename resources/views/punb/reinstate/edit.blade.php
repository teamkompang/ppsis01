...
@auth
						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item"><a href="{{ route('punbreinstate.index') }}">Search</a></li>
												...
												<div class="pd-20">
													...
													<div class="clearfix mb-20">
														...
														<div class="pull-right">
															...
															<a href="{{route('punbreinstate.edit', [
																//$reinstates->ID, 
																$id, 
																'output' => 'print',
															...
															@else
																@if( $reinstates->ICode == Auth::user()->company ||  Auth::user()->company == "PUNB" )
																	@can('punb_reinstate-create')
																		<button type="button" class="btn btn-primary" id="new_line" href="#" 
													...
													@if ($statuscase->count())
														<!-- disbaled -->
														<!-- <form action="{{ route('punbreinstate.update', $reinstates->ID )  }}" method="POST" name="updatehead">
															@csrf
															@method('PATCH')
															<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
															<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $reinstates->ID }}">
													...
													@else  
														@if( $reinstates->ICode == Auth::user()->company ||  Auth::user()->company == "PUNB" )
															@can('punb_reinstate-delete')
																<!-- active -->
																<form action="{{ route('punbreinstate.update', $reinstates->sis_id )  }}" method="POST" name="updatehead">
																	@csrf
																	@method('PATCH')
																	<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																	<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $reinstates->ID }}">
	...
	@include('modal/punb/reinstate/modalshowcase')
	@include('modal/punb/reinstate/modalreinstate')
	@include('modal/punb/reinstate/modalupload')
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{ route('punbreinstate.edit', $id) }}",
				columns: [
		...
		$('body').on('click', '.ActiveItem', function () {
			...
			if (confirm("Are You sure want to unhide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('RINstatusActiveCase',$id) }}",
		...
		$('body').on('click', '.HideItem', function () {
			...
			if (confirm("Are You sure want to hide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('RINstatusHideCase',$id) }}",
					data: { 'Item_id' : Item_id, 'user_lastmaintain' : user_lastmaintain },
					success: function (data) {
						location.reload();
					},
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/ViewCommentReinst/'+comm_id, function (data) {
