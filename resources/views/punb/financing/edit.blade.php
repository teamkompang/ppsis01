												...
												@else  
													@if( $financings->ICode == Auth::user()->company ||  Auth::user()->company == "PUNB" )
														@can('punb_financing-delete')
															<!-- active -->
															<form action="{{ route('punbfinancing.update', $financings->sis_id )  }}" method="POST" name="updatehead">
																@csrf
																@method('PATCH')
																<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $financings->ID }}">
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{ route('punbfinancing.edit', $id) }}",
				columns: [
		...
		$('body').on('click', '.ActiveItem', function () {
			...
			if (confirm("Are You sure want to unhide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('STLstatusActiveCase',$id) }}",
		...
		$('body').on('click', '.HideItem', function () {
			...
			if (confirm("Are You sure want to hide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('STLstatusHideCase',$id) }}",
					data: { 'Item_id' : Item_id, 'user_lastmaintain' : user_lastmaintain },
					success: function (data) {
						location.reload();
					},
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/ViewComment/'+comm_id, function (data) {
