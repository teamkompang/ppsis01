...
@auth
											...
											<div class="pd-20">
												...
												<div class="clearfix mb-20">
													...
													<div class="pull-right">
														...
														<a href="{{route('panelnewfinancing.edit', [
															//$financings->ID, 
															$id, 
															'output' => 'print',
												...
												@if ($statuscase->count())
													<!-- disbaled -->
													<!-- <form action="{{ route('panelnewfinancing.update', $financings->ID )  }}" method="POST" name="updatehead">
														@csrf
														@method('PATCH')
														<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
														<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $financings->ID }}">
												...
												@else  
													@if( $financings->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
														@can('panel_financing-delete')
															<!-- active -->
															<form action="{{ route('panelnewfinancing.update', $financings->sis_id )  }}" method="POST" name="updatehead">
																@csrf
																@method('PATCH')
																<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $financings->ID }}">
	...
	@include('modal/panel/newfinancing/modalfinancings')
	@include('modal/panel/newfinancing/modalupload')
	@include('modal/panel/newfinancing/modalshowcase')
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{ route('panelnewfinancing.edit', $id) }}",
				columns: [
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/PanelViewCommentNF/'+comm_id, function (data) {
