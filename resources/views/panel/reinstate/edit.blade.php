...
@auth
						...
						<div class="col-md-6 col-sm-12">
							...
							<nav aria-label="breadcrumb" role="navigation">
								<ol class="breadcrumb">
                                    ...
                                    <li class="breadcrumb-item active" aria-current="page">Reinstatement</li>
								</ol>
							</nav>
						</div>
					</div>
							...
							<div class="row">
								<div class="col-md-6 col-sm-12">
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="SIS{{ sprintf("%010d",$reinstates->sis_id) }}" disabled>
									</div>
									<label class="col-sm-12 col-md-12 col-form-label">panel Officer</label>
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->LADOFFICER }}" disabled>
									</div>
								</div>
								<div class="col-md-6 col-sm-12">
									<label class="col-sm-12 col-md-12 col-form-label">Case Status</label>
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->value_details }}" disabled>
									</div>
									<label class="col-sm-12 col-md-12 col-form-label">Panel Solicitors</label>
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->Name }}" disabled>
									</div>
								</div>
						...
						<form>
							<div class="row">
								<div class="col-md-6 col-sm-12">
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->FULLNAME }}" disabled>
									</div>
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ date('d-m-Y', strtotime($reinstates->DTMEETING))  }}" disabled>
									</div>
									<label class="col-sm-12 col-md-12 col-form-label">Application Type</label>
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->description }}" disabled>
									</div>
									<label class="col-sm-12 col-md-12 col-form-label">Reinstatement Track</label>
									<div class="col-sm-12 col-md-10">
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->FACSTATUS }}" disabled>
									</div> -->
								</div>
								<div class="col-md-6 col-sm-12">
									<label class="col-sm-12 col-md-12 col-form-label">Facility</label>
									<div class="col-sm-12 col-md-10">
										<textarea class="form-control" disabled>{{ $reinstates->case_no }}</textarea>
									</div>
									<label class="col-sm-12 col-md-12 col-form-label">Approval Committee</label>
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $reinstates->MEETINGNAME }}" disabled>
									</div>
												...
												<div class="pd-20">
													...
													<div class="clearfix mb-20">
														...
														<div class="pull-right">
															...
															<a href="{{route('panelreinstate.edit', [
																//$reinstates->ID, 
																$id, 
																'output' => 'print',
																'page' => $com_reins->currentPage()
															...
															@else
																@if( $reinstates->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
																	@can('panel_reinstate-create')
																		<button type="button" class="btn btn-primary" id="new_line" href="#" 
													...
													@if ($statuscase->count())
														<!-- disbaled -->
														<!-- <form action="{{ route('panelreinstate.update', $reinstates->ID )  }}" method="POST" name="updatehead">
															@csrf
															@method('PATCH')
															<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
															<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $reinstates->ID }}">
													...
													@else  
														@if( $reinstates->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
															@can('panel_reinstate-delete')
																<!-- active -->
																<form action="{{ route('panelreinstate.update', $reinstates->sis_id )  }}" method="POST" name="updatehead">
																	@csrf
																	@method('PATCH')
																	<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																	<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $reinstates->ID }}">
	...
	@include('modal/panel/reinstate/modalreinstate')
	@include('modal/panel/reinstate/modalupload')
	@include('modal/panel/reinstate/modalshowcase')
	...
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{ route('panelreinstate.edit', $id) }}",
				columns: [
		...
		$('body').on('click', '.ActiveItem', function () {
			...
			if (confirm("Are You sure want to unhide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('PRINstatusActiveCase',$id) }}",
		...
		$('body').on('click', '.HideItem', function () {
			...
			if (confirm("Are You sure want to hide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('PRINstatusHideCase',$id) }}",
					data: { 'Item_id' : Item_id, 'user_lastmaintain' : user_lastmaintain },
					success: function (data) {
						location.reload();
					},
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/PanelViewCommentReinst/'+comm_id, function (data) {
