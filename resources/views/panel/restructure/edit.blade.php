...
@auth
						...
						<form>
							<div class="row">
								<div class="col-md-6 col-sm-12">
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $restructures->case_no}}" disabled>
									</div> -->
											...
											<div class="pd-20">
												...
												<div class="clearfix mb-20">
													...
													<div class="pull-right">
														...
														<a href="{{route('panelrestructure.edit', [
															//$restructures->ID, 
															$id, 
															'output' => 'print',
														...
														@else
															@if( $restructures->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
																@can('panel_restructure-create')
																	<button type="button" class="btn btn-primary" id="new_line" href="#" 
												...
												@if ($statuscase->count())
													<!-- disbaled -->
													<!-- <form action="{{ route('panelrestructure.update', $restructures->ID )  }}" method="POST" name="updatehead">
														@csrf
														@method('PATCH')
														<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
														<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $restructures->ID }}">
												...
												@else  
													@if( $restructures->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
														@can('panel_restructure-delete')
															<!-- active -->
															<form action="{{ route('panelrestructure.update', $restructures->sis_id )  }}" method="POST"  name="updatehead">
																@csrf
																@method('PATCH')
																<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $restructures->ID }}">
																...
																<right><button type="submit" class="btn btn-primary" id="close_case" >Request Close Case</button></right>
															</form>
														@endcan	
													@else
	...
	@include('modal/panel/restructure/modalrestructure')
	@include('modal/panel/restructure/modalupload')
	@include('modal/panel/restructure/modalshowcase')
	<script type="text/javascript">
		...
		$(function () {
			...
			var table = $('.data-table').DataTable({
				processing: true,
				serverSide: true,
				ajax: "{{ route('panelrestructure.edit', $id) }}",
				columns: [
		...
		$('body').on('click', '.ActiveItem', function () {
			...
			if (confirm("Are You sure want to hide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('PRSTstatusActiveCase',$id) }}",
		...
		$('body').on('click', '.HideItem', function () {
			...
			if (confirm("Are You sure want to hide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('PRSTstatusHideCase',$id) }}",
					data: { 'Item_id' : Item_id, 'user_lastmaintain' : user_lastmaintain },
					success: function (data) {
						location.reload();
					},
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/PanelViewCommentRestruc/'+comm_id, function (data) {
