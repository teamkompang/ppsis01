...
@auth
						...
						<form>
							<div class="row">
								...
								<div class="col-md-6 col-sm-12">
									...
									<div class="col-sm-12 col-md-10">
										<input class="form-control form-control-sm form-control-line" type="text" value="{{ $vlos->value_details }}" disabled>
									</div>
											...
											<div class="pd-20">
												...
												<div class="clearfix mb-20">
													...
													<div class="pull-right">
														...
														@else
															@if( $vlos->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
																@can('panel_vlo-create')
																	<button type="button" class="btn btn-primary" id="new_line" href="#" 
												...
												@if ($statuscase->count())
													<!-- disbaled -->
													<!-- <form action="{{ route('panelvlo.update', $vlos->ID )  }}" method="POST" name="updatehead">
														@csrf
														@method('PATCH')
														<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
														<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $vlos->ID }}">
												...
												@else  
													@if( $vlos->ICode == Auth::user()->company ||  Auth::user()->company == "panel" )
														@can('panel_vlo-delete')
															<!-- active -->
															<form action="{{ route('panelvlo.update', $vlos->sis_id )  }}" method="POST" name="updatehead">
																@csrf
																@method('PATCH')
																<input class="form-control form-control-sm form-control-line" type="hidden" name="user_lastmaintain" value="{{ Auth::user()->fullname }}">
																<input class="form-control form-control-sm form-control-line" type="hidden" name="header_id" value="{{ $vlos->ID }}">
	...
	@include('modal/panel/vlo/modalvlos')
	@include('modal/panel/vlo/modalupload')
	@include('modal/panel/vlo/modalshowcase')
	...
	<script type="text/javascript">
		...
		$('body').on('click', '.ActiveItem', function () {
			...
			if (confirm("Are You sure want to unhide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('PVLOstatusActiveCase',$id) }}",
		...
		$('body').on('click', '.HideItem', function () {
			...
			if (confirm("Are You sure want to hide comment ?") == true){
				$.ajax({
					type: "POST",
					url: "{{ route('PVLOstatusHideCase',$id) }}",
					data: { 'Item_id' : Item_id, 'user_lastmaintain' : user_lastmaintain },
					success: function (data) {
						location.reload();
					},
		...
		$('body').on('click', '#show-user', function () {
			...
			console.log(comm_id);
			$.get('/PanelViewCommentVlo/'+comm_id, function (data) {
